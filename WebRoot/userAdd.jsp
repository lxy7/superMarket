...
function checkName()
{
	var name = document.form1.username.value;//获取输入框的 
	  ...
	  if(xhr.readyState==4)
		  {
		    if(xhr.status==200)
		    	var msg = xhr.responseText
		    	{
		    	  if(msg=='no'&& name !='' )
		    		  {
		    		   flag.innerHTML="<font color= 'green'>用户名可以使用</font>";
		    		  }
		    	  if(msg=='yes'&& name !='' )
		    		  {
		    		  flag.innerHTML="<font color= 'red'>用户名已存在</font>";
		    		  }
		    	  if(name =='')
		    		  {
		    		  flag.innerHTML="<font color=></font>";
		    		  }
		    	}
		  }
  }
  xhr.open("get","${pageContext.request.contextPath}/user?action=isExist&username="+name);
  xhr.send(null);
  }
</script>
</head>
...
<div class="main">
	...
	<form id="form1" name="form1" method="post"    action="/SuperMarket/user?action=adduser" >
<input type="hidden" name="flag" value="doAdd">
		<div class="content">
			...
			<tr>
					...
					<td><input type="text" name="username" class="text" id="textfield2" onblur="checkName()" /> <font color="red">*</font><span id="msg"></span></td>
				</tr>
				<tr>
    ...
    <option value="1">男</option>
		...
		<div class="buttons">
			<input type="submit" name="button" id="button" value="数据提交" class="input-button"/>
			  <input type="button" name="button" id="button" onclick="window.location='/${pageContext.request.contextPath}/user?action=user';" value="返回" class="input-button"/> 
		</div>
